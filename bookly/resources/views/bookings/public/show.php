...
/* === Container === */
.dm-container { max-width: 640px; margin: 0 auto; padding: 16px; }
@media (min-width: 640px) { .dm-container { padding: 24px; } }
@media (min-width: 1024px) { .dm-container { padding: 32px 24px; } }

/* === Header === */
.dm-header { text-align: center; margin-bottom: 24px; }
.dm-logo { display: inline-grid; place-items: center; width: 64px; height: 64px; border-radius: 20px; background: linear-gradient(135deg, #0071E3, #5AC8FA); color: #fff; font-size: 26px; font-weight: 700; margin-bottom: 12px; box-shadow: 0 8px 24px rgba(0,113,227,0.25); }
@media (min-width: 640px) { .dm-logo { width: 80px; height: 80px; font-size: 30px; border-radius: 24px; } }
.dm-title { font-size: 24px; font-weight: 600; letter-spacing: -0.02em; margin: 0; color: #1D1D1F; }
@media (min-width: 640px) { .dm-title { font-size: 30px; } }
.dm-sub { color: rgba(0,0,0,0.5); margin: 4px 0 0; font-size: 14px; }

/* === Stepper === */
.dm-stepper { display: flex; align-items: center; margin-bottom: 12px; }
...
@media (max-width: 479px) { .dm-step-lbl { display: none; } }
...
@media (min-width: 640px) { .dm-dot { width: 32px; height: 32px; font-size: 13px; } }
...
.dm-conn { flex: 1; height: 2px; background: #E5E5EA; margin: 0 6px; border-radius: 999px; overflow: hidden; min-width: 8px; }
...
.dm-conn.partial .dm-conn-fill { width: 50%; }

/* === Bar === */
.dm-bar { position: relative; height: 4px; background: #E5E5EA; border-radius: 999px; overflow: hidden; margin-bottom: 24px; }
...
@media (min-width: 640px) { .dm-bar { margin-bottom: 32px; } }

/* === Card === */
.dm-card { background: #fff; border-radius: 20px; box-shadow: 0 4px 24px rgba(0,0,0,0.06); padding: 20px; }
@media (min-width: 640px) { .dm-card { padding: 32px; border-radius: 24px; } }
@media (min-width: 1024px) { .dm-card { padding: 40px; } }
...
.dm-stp-h { font-size: 18px; font-weight: 600; margin: 0 0 4px; color: #1D1D1F; }
@media (min-width: 640px) { .dm-stp-h { font-size: 22px; } }
.dm-stp-sub { font-size: 14px; color: rgba(0,0,0,0.5); margin: 0 0 20px; }

/* === Service buttons === */
.dm-svc-list { display: flex; flex-direction: column; gap: 8px; }
.dm-svc { width: 100%; text-align: left; background: #fff; border: 1.5px solid rgba(0,0,0,0.08); border-radius: 16px; padding: 16px; cursor: pointer; transition: all .2s cubic-bezier(.16,1,.3,1); display: flex; align-items: center; gap: 12px; min-height: 64px; -webkit-tap-highlight-color: transparent; }
@media (min-width: 640px) { .dm-svc { padding: 18px 20px; gap: 16px; min-height: 72px; } }
.dm-svc:hover { border-color: #0071E3; background: #F5F9FF; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(0,113,227,0.08); }
.dm-svc:active { transform: scale(0.99); }
.dm-svc.sel { border-color: #0071E3; background: #E8F3FF; box-shadow: 0 0 0 3px rgba(0,113,227,0.1); }
.dm-svc-emoji { font-size: 28px; flex-shrink: 0; width: 48px; height: 48px; display: grid; place-items: center; background: #F5F5F7; border-radius: 12px; }
@media (min-width: 640px) { .dm-svc-emoji { font-size: 32px; width: 56px; height: 56px; } }
.dm-svc-info { flex: 1; min-width: 0; }
.dm-svc-name { font-weight: 600; font-size: 15px; color: #1D1D1F; line-height: 1.3; }
@media (min-width: 640px) { .dm-svc-name { font-size: 16px; } }
.dm-svc-meta { font-size: 13px; color: rgba(0,0,0,0.5); margin-top: 2px; }
.dm-svc-price { font-weight: 700; font-size: 17px; color: #0071E3; flex-shrink: 0; }
@media (min-width: 640px) { .dm-svc-price { font-size: 18px; } }
.dm-svc-check { width: 24px; height: 24px; border-radius: 999px; border: 2px solid rgba(0,0,0,0.1); display: grid; place-items: center; flex-shrink: 0; transition: all .2s; color: transparent; }
.dm-svc.sel .dm-svc-check { background: #0071E3; border-color: #0071E3; color: #fff; }

/* === Time slots === */
.dm-slots-wrap { position: relative; }
.dm-slots-loading { color: rgba(0,0,0,0.5); font-size: 14px; padding: 20px 0; display: flex; align-items: center; gap: 8px; }
...
.dm-day-group { margin-bottom: 16px; }
.dm-day-group:last-child { margin-bottom: 0; }
.dm-day-lbl { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: rgba(0,0,0,0.4); margin-bottom: 8px; padding-left: 4px; }
.dm-slots { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }
@media (min-width: 480px) { .dm-slots { grid-template-columns: repeat(4, 1fr); gap: 10px; } }
@media (min-width: 768px) { .dm-slots { grid-template-columns: repeat(5, 1fr); } }
@media (min-width: 1024px) { .dm-slots { grid-template-columns: repeat(6, 1fr); } }
.dm-slot { padding: 12px 8px; border-radius: 12px; border: 1.5px solid rgba(0,0,0,0.08); background: #fff; cursor: pointer; font-weight: 600; font-size: 14px; transition: all .15s; min-height: 44px; -webkit-tap-highlight-color: transparent; text-align: center; }
@media (min-width: 640px) { .dm-slot { padding: 14px 8px; font-size: 15px; min-height: 48px; } }
.dm-slot:hover { border-color: #0071E3; background: #F5F9FF; }
.dm-slot:active { transform: scale(0.96); }
.dm-slot.sel { background: #0071E3; border-color: #0071E3; color: #fff; box-shadow: 0 4px 12px rgba(0,113,227,0.25); }

/* === Date input === */
.dm-date { width: 100%; padding: 14px 16px; border-radius: 14px; border: 1.5px solid #E5E5EA; font-size: 16px; box-sizing: border-box; background: #fff; transition: all .2s; min-height: 52px; -webkit-appearance: none; appearance: none; }
...
@media (min-width: 640px) { .dm-form-grid { grid-template-columns: 1fr 1fr; } }
.dm-field { display: flex; flex-direction: column; }
.dm-lbl { display: block; font-size: 13px; font-weight: 500; margin-bottom: 6px; color: #1D1D1F; }
.dm-inp { width: 100%; padding: 14px 16px; border-radius: 12px; border: 1.5px solid #E5E5EA; font-size: 15px; box-sizing: border-box; background: #fff; transition: all .2s; min-height: 48px; font-family: inherit; }
.dm-inp:focus { outline: none; border-color: #0071E3; box-shadow: 0 0 0 3px rgba(0,113,227,0.15); }
textarea.dm-inp { min-height: 80px; resize: vertical; }
.dm-inp::placeholder { color: rgba(0,0,0,0.3); }

/* === Buttons === */
.dm-actions { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-top: 24px; flex-wrap: wrap; }
@media (max-width: 479px) { .dm-actions { flex-direction: column-reverse; } .dm-actions > * { width: 100%; justify-content: center; } }
.dm-back { background: none; border: none; color: rgba(0,0,0,0.5); font-size: 14px; font-weight: 500; cursor: pointer; padding: 12px 16px; min-height: 44px; border-radius: 12px; transition: all .2s; display: inline-flex; align-items: center; gap: 6px; -webkit-tap-highlight-color: transparent; }
.dm-back:hover { background: rgba(0,0,0,0.05); color: #1D1D1F; }
.dm-next { background: #0071E3; color: #fff; padding: 14px 28px; border-radius: 999px; font-weight: 600; border: none; cursor: pointer; font-size: 15px; display: inline-flex; align-items: center; gap: 8px; min-height: 48px; transition: all .2s; -webkit-tap-highlight-color: transparent; box-shadow: 0 2px 8px rgba(0,113,227,0.2); }
.dm-next:hover { background: #0066CC; transform: translateY(-1px); box-shadow: 0 6px 16px rgba(0,113,227,0.3); }
.dm-next:active { transform: scale(0.98); }
.dm-next:disabled { opacity: 0.4; cursor: not-allowed; transform: none; box-shadow: none; }
.dm-next svg { transition: transform .2s; }
.dm-next:hover:not(:disabled) svg { transform: translateX(2px); }
.dm-back svg { transition: transform .2s; }
.dm-back:hover svg { transform: translateX(-2px); }

/* === Footer === */
.dm-foot { text-align: center; font-size: 12px; color: rgba(0,0,0,0.4); margin-top: 32px; }
...
<div class="dm-container" id="booklyDemo">
  <!-- Language switcher -->
  ...
  <div class="dm-card">
    ...
    <div class="dm-stp on" data-step="1">
      ...
      <div class="dm-actions">
        <span></span>
        <button type="button" class="dm-next" id="dmGoStep2" disabled>
          <?= t('book.next') ?>
    ...
    <div class="dm-stp" data-step="2">
      ...
      <div class="dm-actions">
        <button type="button" class="dm-back" data-go="1">
        ...
        <button type="button" class="dm-next" id="dmGoStep3">
          <?= t('book.next') ?>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 4l4 4-4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </button>
      </div>
    ...
    <div class="dm-stp" data-step="3">
      ...
      <div class="dm-actions">
        <button type="button" class="dm-back" data-go="2">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 4l-4 4 4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          <?= t('book.back') ?>
        </button>
        <span></span>
    ...
    <div class="dm-stp" data-step="4">
      ...
      <form method="POST" action="/book/<?= e($business['slug']) ?>" id="dmForm">
        ...
        <div class="dm-actions">
          <button type="button" class="dm-back" data-go="3">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 4l-4 4 4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <?= t('book.back') ?>
          </button>
          <button type="submit" class="dm-next">
            <?= t('book.confirm') ?>
...
(function() {
  ...
  function go(n) {
    ...
    // Scroll to top of card on mobile when advancing
    if (n > prev && window.innerWidth < 768) {
      var card = $('.dm-card', root);
